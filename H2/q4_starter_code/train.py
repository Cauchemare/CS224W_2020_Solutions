...
def train(dataset,task, args):
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % 10 == 0:
            ...
            print(test_acc,'  test')
        valid_acc = test(loader,model,True)
        valid_accs.append(valid_acc)
    plot(valid_accs,args)
...
def plot(accs,args):
    ...
    plt.plot(x,accs)
    plt.title('{} :{}'.format(args.dataset,args.model_type))
    plt.show()
...
def main():
    ...
    if args.dataset == 'enzymes':
        dataset = TUDataset(root='F:/', name='ENZYMES')
        task = 'graph'
    elif args.dataset == 'cora':
        dataset = Planetoid(root='F:/', name='Cora')
        task = 'node'
